...
public interface DetailedListMapper {
	...
	List<DetailedListVo> SelectDetailedListByUserId(@Param("userId") String userId);
	...
	public int InsertDetailedListInfoDy(DetailedList detailedList);
	...
	@Update("UPDATE detailed_list SET need_number = #{needNumber} "
			+ "where detailed_list.commodity_number = #{commodityNumber}" + " And detailed_list.user_id = #{userId}")
	int UpdateCommodityByUserId(DetailedList detailedList);

	/*
	 * int UpdateCommodityByUserId (@Param("needNumber") int needNumber
	 * ,@Param("commodityNumber") int commodityNumber ,@Param("userId") String
	 * userId);
	 */
